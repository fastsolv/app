                                ...
                                <table class="table table-striped" id="table-1">
                                    <thead>
                                        <tr class="text-center text-capitalize">
                                            <th><a class="text-secondary text-decoration-none font-weight-bold"
                                                    href="{{ route('get_kb_category', ['name' => 'name', 'order' => $sort_order]) }}">{{ __('Name') }}
                                                    <span>
                                                        @if ($sort_order == 'asc')
                                                ...
                                                @if ($show_edit_button || $show_delete_button)
                                                    <td class="justify-content-center form-inline">
                                                        ...
                                                        @if ($show_delete_button)
                                                            <form
                                                                action="{{ route('kb_category.destroy', [$kb_category->uuid]) }}"
                                                                method="POST">
                                                                @method('DELETE')
                                                                @csrf
