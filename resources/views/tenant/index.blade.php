...
<div class="section-header">
    ...
    <div class="section-header-breadcrumb">
        ...
        <div class="breadcrumb-item">{{ __('Email Tickets') }}</div>
    </div>
</div>
        ...
        <div class="col-12">
            ...
            <div class="card">
                ...
                <div class="card-header">
                    <h4 class="inline-block">{{ __('List of Email Tickets') }}</h4>
                </div>
                <div class="card-body">
                    <form action="/imap_ticket" method="get">
                        @csrf
                    ...
                    <div class="table-responsive pt-1">
                        ...
                        <li class="nav-item">
                                <a class="nav-link active"
                                    href="{{ route('get_imap_ticket') }}"></i><span>{{ __('All Tickets') }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('imapTickets', 8) }}"><span>{{ __('Awaiting/New') }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('imapTickets', 6) }}"><span>{{ __('Awaitings') }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('imapTickets', 1) }}"><span>{{ __('Opened') }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('imapTickets', 5) }}"><span>{{ __('Answered') }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('imapTickets', 3) }}"><span>{{ __('In Progress') }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('imapTickets', 4) }}"><span>{{ __('On Hold') }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('imapTickets', 2) }}"><span>{{ __('Closed') }}</span></a>
                            </li> 
                           
                            <!-- @foreach ($statuses as $status)
                            <li class="nav-item">
                                <a class="nav-link"
                                    href="{{ route('imapTickets', [$status->id]) }}"><span>{{ __($status->title) }}</span></a>
                            </li>
                            @endforeach -->
                        </ul>
                        <div class="user-ticket-divider"></div>
                        @if (!count($imap_tickets))
                        ...
                        <table class="table table-striped pt-3" id="table-1">
                            <thead>
                                ...
                                <th><a  class ="text-secondary text-decoration-none font-weight-bold" href="{{ route('get_imap_ticket',['name' => 'tid' ,'order'=>$sort_order]) }}">{{ __('Ticket ID') }} 
                                    ...
                                    <th><a  class ="text-secondary text-decoration-none font-weight-bold" href="{{ route('get_imap_ticket',['name' => 'opened_user_id' ,'order'=>$sort_order]) }}">{{ __('customer') }} 
                                        <span> @if($sort_order =='asc') 
                                                <i  class=" fa fa-sort-alpha-up mt-1 float-right  "></i>
                                                    @else
                                                    <i  class=" mt-1 float-right   fa fa-sort-alpha-down  "></i>
                                    ...
                                    <th><a  class ="text-secondary text-decoration-none font-weight-bold" href="{{ route('get_imap_ticket',['name' => 'title' ,'order'=>$sort_order]) }}">{{ __('Subject') }} 
                                        <span> @if($sort_order =='asc') 
                                                <i  class=" fa fa-sort-alpha-up mt-1 float-right  "></i>
                                                    @else
                                                    <i  class=" mt-1 float-right   fa fa-sort-alpha-down  "></i>
                                    ...
                                    <th><a  class ="text-secondary text-decoration-none font-weight-bold" href="{{ route('get_imap_ticket',['name' => 'ticket_status_id' ,'order'=>$sort_order]) }}">{{ __('Status') }} 
                                        <span> @if($sort_order =='asc') 
                                                <i  class=" fa fa-sort-alpha-up mt-1 float-right  "></i>
                                                    @else
                                                    <i  class=" mt-1 float-right   fa fa-sort-alpha-down  "></i>
                                    ...
                                    <th><a  class ="text-secondary text-decoration-none font-weight-bold" href="{{ route('get_imap_ticket',['name' => 'ticket_urgency_id' ,'order'=>$sort_order]) }}">{{ __('Priority') }} 
                                        <span> @if($sort_order =='asc') 
                                                <i  class=" fa fa-sort-alpha-up mt-1 float-right  "></i>
                                                    @else
                                                    <i  class=" mt-1 float-right   fa fa-sort-alpha-down  "></i>
                                    ...
                                    <th><a  class ="text-secondary text-decoration-none font-weight-bold" href="{{ route('get_imap_ticket',['name' => 'department_id' ,'order'=>$sort_order]) }}">{{ __('Department') }} 
                                        <span> @if($sort_order =='asc') 
                                                <i  class=" fa fa-sort-alpha-up mt-1 float-right  "></i>
                                                    @else
                                                    <i  class=" mt-1 float-right   fa fa-sort-alpha-down  "></i>
                                    ...
                                    <th><a  class ="text-secondary text-decoration-none font-weight-bold" href="{{ route('get_imap_ticket',['name' => 'last_touched_at' ,'order'=>$sort_order]) }}">{{ __('Last action') }} 
                                        <span> @if($sort_order =='asc') 
                                                <i  class=" fa fa-sort-alpha-up mt-1 float-right  "></i>
                                                    @else
                                                    <i  class=" mt-1 float-right   fa fa-sort-alpha-down  "></i>
                            ...
                            <tbody>
                                @foreach ($imap_tickets as $ticket)

                                @if ($ticket->ticketUnread )
                                <tr class="bg-table-custom font-weight-bold-custom">
                                    <td>
                                        <a class=""
                                        href="{{ route('ticket.reply', [$ticket->uuid]) }}">#{{ $ticket->tid}} </a>
                                    </td>
                                    <td><a class="font-weight-bold-custom"
                                            href="{{ route('imap_ticket.reply', [$ticket->uuid]) }}">{{ Str::limit($ticket->subject, 30) }}</a>
                                        @if (count($ticket->tags))
                                        <br>
                                        @foreach ($ticket->tags as $tag)
                                        <span class="tag-badge badgesize text-white"
                                            style="color: {{ $tag->text_color }};background-color: {{ $tag->tag_color }} !important;">
                                            {{$tag->name}}</span>
                                        @endforeach
                                        @endif
                                    </td>
                        ...
                        <br>
                        {{ $imap_tickets->appends($request->all())->links("pagination::bootstrap-4") }}
