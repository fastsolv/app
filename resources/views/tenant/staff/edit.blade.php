                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('staff.update', $id) }}">
                        ...
                        <div>
                            ...
                            <div class="form-group row mb-4">
                                <label
                                    class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('Position') }}:</label>
                                <div class="col-sm-12 col-md-7">
                                <select class="form-control selectric" id="role_id" name="role_id">
                                    @foreach($roles as $role)
                                    @if ($role->uuid == old('role_id', $role_id))
                                    <option selected value="{{$role->uuid}}">
                                        {{__($role->name)}}</option>
                                    @else
                                    <option value="{{$role->uuid}}">{{__($role->name)}}</option>
                                    @endif
                                    @endforeach
                                </select>
