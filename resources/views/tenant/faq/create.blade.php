                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('faq.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group row mb-4">
                            <label for="address"
                                class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('Question for') }}  {{ $language->language }}*</label>
                            <div class="col-sm-12 col-md-7">
                                <input id="custom[{{$language->id}}]" type="text"
                                    class="form-control @error('custom.'.$language->id.'.question') is-invalid @enderror" name="custom[{{$language->id}}][question]"
                        ...
                        <div class="form-group row mb-4">
                            <label
                                class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('Answer for') }} {{ $language->language}}:*</label>
                            <div class="col-sm-12 col-md-7">
                                <textarea id="custom[{{$language->id}}]" class="summernote @error('custom.'.$language->id.'.answer') is-invalid @enderror"
                                     name="custom[{{$language->id}}][answer]">{{ old('answer') }}</textarea>
                                @error('custom[{{$language->id}}]')
                                <div class="text-danger pt-1">{{ $message }}</div>
                                @enderror
                            </div>
