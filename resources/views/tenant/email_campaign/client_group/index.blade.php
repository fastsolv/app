        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header">
                    <h4 class="inline-block">{{ __('Client Group Email Campaign') }}</h4>
                    <a href="{{ route('client_group_campaign.create') }}"
                        class="btn btn-icon btn-custom float-right inline-block"><i
                            class="far fa-edit"></i>{{ __('Add') }}</a>
                </div>
                <div class="card-body">
                    <div class="search-bar inline-block">
                        <form action="/client_group_campaign" method="get">
                            ...
                            <tbody>
                                ...
                                <tr class="text-capitalize">
                                    ...
                                    <td class="justify-content-center form-inline">
                                        @if($email_campaign->status != "done")
                                        <a href="{{ route('client_group_campaign.edit', [$email_campaign->uuid]) }}"
                                            class="btn btn-sm bg-transparent"><i class="far fa-edit text-primary"
                                                aria-hidden="true" title="{{ __('Edit') }}"></i></a>
                                        @endif
                                        <form action="{{ route('client_group_campaign.destroy', [$email_campaign->uuid]) }}"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
