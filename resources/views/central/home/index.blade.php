    ...
    <div class="section-header-breadcrumb">
        <div class="breadcrumb-item">
            ...
            <div class="dropdown-menu">
                ...
                <a class="dropdown-item" href={{ route('plan_details.index', ['currency' => $currency->currency]) }}>{{ $currency->currency }}</a>
                @endforeach
            </div>
        </div>
        <div class="breadcrumb-item">
            ...
            <div class="dropdown-menu">
                <a class="dropdown-item {{ $selectedPeriod == 'Monthly' ? 'active' : '' }}" href="{{ route('plan_details.index', ['currency' => $selectedCurrency, 'period' => 'Monthly']) }}">Monthly</a>
                <a class="dropdown-item {{ $selectedPeriod == 'Yearly' ? 'active' : '' }}" href="{{ route('plan_details.index', ['currency' => $selectedCurrency, 'period' => 'Yearly']) }}">Yearly</a>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="borderless">
                            <li class="list-group-item d-flex justify-content-between align-items-center border-0">
                                ...
                                <span >{{ $plan->department_count ? $plan->department_count : 'Unlimited' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center border-0">
                                {{ __('No. of tickets') }}
                            ...
                            @if ($price->uuid == $plan->uuid)  
                                @if ($plan->require_payment == 0)
                                    <a type="button" href="{{ route('register_minimal_user', ['id' => $price->id] ) }}"
                                        class="btn btn-custom btn-block pricing-button-new">{{ __('Subscribe') }}</a>
                                @else
                                    @if ($price->period == $selectedPeriod)
                                        <a href="{{ route('register', ['plan' => $price->id]) }}" class="btn btn-custom btn-block pricing-button-new"
